...
in
{
  specialisation.light.configuration = lib.mkMerge [
    { system.nixos.tags = [ "light" ]; environment.variables.NIXOS_SPECIALISATION = "light"; }
    lightTheme
  ];

  specialisation.sync.configuration = lib.mkMerge [
    { system.nixos.tags = [ "sync" ]; environment.variables.NIXOS_SPECIALISATION = "sync"; }
    nvidiaSync
  ];

  specialisation.light-sync.configuration = lib.mkMerge [
    { system.nixos.tags = [ "light-sync" ]; environment.variables.NIXOS_SPECIALISATION = "light-sync"; }
    lightTheme
    nvidiaSync
  ];
}
