...
}:
{
  ...
  sops.templates."rclone-google-drive.env".content = ''
    ...
    RCLONE_CONFIG_GOOGLE_DRIVE_CLIENT_SECRET=${
      config.sops.placeholder."rclone/google_drive_client_secret"
    }
  ...
  sops.templates."rclone-proton-drive.env".content = ''
    ...
    RCLONE_CONFIG_PROTON_DRIVE_OTP_SECRET_KEY=${
      config.sops.placeholder."rclone/proton_drive_otp_secret_key"
    }
  '';
